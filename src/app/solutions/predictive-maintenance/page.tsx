...
import { ArrowRight, Settings, Shield, CheckCircle, Zap, Search, Database, Workflow, Target, Layers, Cpu, Cloud, BarChart, AlertTriangle, Wrench, Activity, Eye, type LucideIcon } from "lucide-react";
...
const iconMap: Record<IconName, LucideIcon> = { settings: Settings, shield: Shield, checkCircle: CheckCircle, zap: Zap, search: Search, database: Database, workflow: Workflow, target: Target, layers: Layers, cpu: Cpu, cloud: Cloud, barChart: BarChart, alertTriangle: AlertTriangle, tool: Wrench, activity: Activity, eye: Eye };
        ...
        <>
            ...
            <main>
                                ...
                                <div className="relative w-full max-w-md z-10">
                                    <div className="flex flex-col justify-between items-center relative h-80">
                                        ...
                                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 flex items-center gap-4 w-64 relative">
                                            <div className="w-10 h-10 bg-blue-50 dark:bg-blue-900/30 rounded flex items-center justify-center text-blue-600"><Wrench className="w-6 h-6" /></div>
                                            <div><div className="text-xs text-primary font-bold uppercase mb-0.5">Output</div><div className="text-sm font-semibold dark:text-white">Smart Work Orders</div></div>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border-2 border-primary flex items-center gap-4 w-72 z-20 scale-110">
                                            <div className="w-12 h-12 bg-primary text-white rounded flex items-center justify-center shadow-lg"><Target className="w-7 h-7" /></div>
                                            <div><div className="text-xs text-gray-400 font-bold uppercase mb-0.5">Core</div><div className="text-base font-bold dark:text-white">Failure Prediction</div></div>
                                        </div>
                                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 flex items-center gap-4 w-64 relative">
                                            ...
                                            <div className="w-10 h-10 bg-green-50 dark:bg-green-900/30 rounded flex items-center justify-center text-green-600"><Activity className="w-6 h-6" /></div>
                                            <div><div className="text-xs text-primary font-bold uppercase mb-0.5">Input</div><div className="text-sm font-semibold dark:text-white">Sensor Telemetry</div></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <Section>
                    <Container>
                        <div className="flex justify-between items-end mb-14">
                            <div><span className="text-primary font-bold uppercase tracking-widest text-sm">Use Cases</span><h2 className="text-3xl font-bold mt-2 text-black dark:text-white">Who Is This For?</h2><p className="mt-4 text-text-muted-light dark:text-text-muted-dark">Manufacturers, energy operators, and fleet managers with critical assets.</p></div>
                ...
                <Section background="surface">
                    <Container>
                        <div className="text-center mb-20"><span className="text-primary font-bold uppercase tracking-widest text-sm">Delivery Process</span><h2 className="text-3xl font-bold mt-2 text-black dark:text-white">How We Deliver Results</h2></div>
                        <ServiceProcessTimeline steps={deliveryPhases.map((s, i) => { const Icon = getIconServer(s.iconName); return { id: i + 1, title: s.title, desc: s.description, iconNode: <Icon /> }; })} />
                    </Container>
                </Section>
                ...
                <Section>
                    <Container>
                        <div className="mb-16 text-center"><span className="text-primary font-bold uppercase tracking-widest text-sm">Architecture</span><h2 className="text-3xl font-bold mt-2 text-black dark:text-white">System Architecture</h2><p className="mt-4 text-text-muted-light dark:text-text-muted-dark">From sensor data to automated work orders.</p></div>
                        <SystemArchitecturePipeline layers={architectureLayers.map((l) => { const Icon = getIconServer(l.iconName); return { title: l.title, subtitle: l.subtitle, iconNode: <Icon /> }; })} pillars={securityControls.map((p) => { const Icon = getIconServer(p.iconName); return { title: p.title, description: p.description, iconNode: <Icon /> }; })} />
                    </Container>
                </Section>
                ...
                <Section background="surface">
                    <Container>
                        <div className="mb-14"><span className="text-primary font-bold uppercase tracking-widest text-sm">Getting Started</span><h2 className="text-3xl font-bold mt-2 text-black dark:text-white">What We Need to Start</h2></div>
                        <div className="grid md:grid-cols-2 gap-8">
                            {inputsRequired.map((input) => {
                                const Icon = getIconServer(input.iconName); return (
                                    <div key={input.title} className="bg-white dark:bg-background-dark p-8 rounded shadow-sm border border-gray-100 dark:border-gray-700 flex items-start gap-5">
                                        <Icon className="w-8 h-8 text-primary flex-shrink-0 mt-1" />
                                        <div><h4 className="text-lg font-bold text-black dark:text-white">{input.title}</h4><p className="text-sm text-gray-600 dark:text-gray-400 mt-2">{input.description}</p></div>
                                    </div>);
                            })}
                        </div>
                    </Container>
                ...
                <Section>
                    <Container>
                        <div className="mb-14 text-center"><span className="text-primary font-bold uppercase tracking-widest text-sm">Success Metrics</span><h2 className="text-3xl font-bold mt-2 text-black dark:text-white">How We Measure Success</h2></div>
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {kpis.map((kpi) => (
                                <div key={kpi.metric} className="bg-white dark:bg-surface-dark p-6 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <h4 className="font-bold text-black dark:text-white mb-2">{kpi.metric}</h4>
                ...
                <Section background="surface">
                    <Container>
                        <div className="flex flex-col items-center mb-16"><h3 className="text-3xl font-bold mb-4 text-black dark:text-white">Technology Stack</h3><p className="text-text-muted-light dark:text-text-muted-dark max-w-2xl text-center">Industry-standard tools for reliable predictive maintenance.</p></div>
                        ...
                        <div className="grid md:grid-cols-3 gap-8">
                            {caseStudies.map((s) => (
                                <div key={s.title} className="bg-white dark:bg-surface-dark rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-shadow border border-gray-100 dark:border-gray-800 flex flex-col group">
                                    <div className="h-52 bg-gray-200 dark:bg-gray-700 relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-br from-gray-600 to-gray-800 group-hover:scale-105 transition-transform duration-500"></div><div className="absolute top-4 left-4 bg-white/95 dark:bg-black/80 px-3 py-1 text-xs font-bold rounded uppercase tracking-wider">Predictive</div></div>
                ...
                <Section className="relative overflow-hidden">
                    ...
                    <Container className="relative z-10">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center"><Button size="lg" href="/contact">Discuss your assets <ArrowRight className="w-4 h-4 ml-2" /></Button><Button variant="outline" size="lg" href="/work">View case studies</Button></div>
                    </Container>
                </Section>
            </main>
            <Footer />
