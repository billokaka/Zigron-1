...
import { ArrowRight, Home, Shield, CheckCircle, Zap, Search, Database, Workflow, Target, Layers, Cpu, Cloud, Smartphone, Wifi, Lock, Eye, Settings, type LucideIcon } from "lucide-react";
...
const iconMap: Record<IconName, LucideIcon> = { home: Home, shield: Shield, checkCircle: CheckCircle, zap: Zap, search: Search, database: Database, workflow: Workflow, target: Target, layers: Layers, cpu: Cpu, cloud: Cloud, smartphone: Smartphone, wifi: Wifi, lock: Lock, eye: Eye, settings: Settings };
function getIconServer(name: IconName): LucideIcon { return iconMap[name] || Home; }
    ...
    return (
        <>
            ...
            <main>
                                ...
                                <div className="relative w-full max-w-md z-10">
                                    <div className="flex flex-col justify-between items-center relative h-80">
                                        ...
                                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 flex items-center gap-4 w-64 relative">
                                            <div className="w-10 h-10 bg-blue-50 dark:bg-blue-900/30 rounded flex items-center justify-center text-blue-600"><Smartphone className="w-6 h-6" /></div>
                                            <div><div className="text-xs text-primary font-bold uppercase mb-0.5">Output</div><div className="text-sm font-semibold dark:text-white">Delightful App</div></div>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border-2 border-primary flex items-center gap-4 w-72 z-20 scale-110">
                                            <div className="w-12 h-12 bg-primary text-white rounded flex items-center justify-center shadow-lg"><Home className="w-7 h-7" /></div>
                                            <div><div className="text-xs text-gray-400 font-bold uppercase mb-0.5">Core</div><div className="text-base font-bold dark:text-white">Smart Home Platform</div></div>
                                        </div>
                                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 flex items-center gap-4 w-64 relative">
                                            ...
                                            <div><div className="text-xs text-primary font-bold uppercase mb-0.5">Input</div><div className="text-sm font-semibold dark:text-white">IoT Devices</div></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <Section background="surface" border="both"><Container><div className="mb-14"><span className="text-primary font-bold uppercase tracking-widest text-sm">Platform Capabilities</span><h2 className="text-3xl font-bold mt-2 text-black dark:text-white">What the Platform Includes</h2></div><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">{modules.map((m) => { const Icon = getIconServer(m.iconName); return (<div key={m.title} className="bg-white dark:bg-background-dark p-8 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 hover:border-primary/30 transition-colors"><Icon className="w-10 h-10 text-primary mb-5" /><h4 className="text-lg font-bold text-black dark:text-white mb-2">{m.title}</h4><p className="text-sm text-gray-600 dark:text-gray-400 mb-4 leading-relaxed">{m.description}</p><ul className="space-y-1.5">{m.features.map((f) => (<li key={f} className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400"><CheckCircle className="w-3.5 h-3.5 text-green-500 flex-shrink-0" />{f}</li>))}</ul></div>); })}</div></Container></Section>

                <Section><Container><div className="flex justify-between items-end mb-14"><div><span className="text-primary font-bold uppercase tracking-widest text-sm">Use Cases</span><h2 className="text-3xl font-bold mt-2 text-black dark:text-white">Who Is This For?</h2></div></div><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">{useCases.map((uc) => (<div key={uc.title} className="bg-white dark:bg-surface-dark p-8 rounded shadow-sm hover:shadow-xl transition border-t-4 border-primary flex flex-col h-full"><h3 className="text-xl font-bold mb-6 text-black dark:text-white">{uc.title}</h3><div className="space-y-5 flex-1"><div className="bg-red-50 dark:bg-red-900/10 p-4 rounded"><p className="text-xs font-bold text-primary mb-1 uppercase tracking-wide">Challenge</p><p className="text-sm text-gray-700 dark:text-gray-300">{uc.problem}</p></div><div className="px-1"><p className="text-xs font-bold text-gray-400 mb-1 uppercase tracking-wide">Approach</p><p className="text-sm text-gray-600 dark:text-gray-400">{uc.solution}</p></div></div><div className="border-t border-gray-100 dark:border-gray-700 pt-4 mt-6"><p className="text-xs font-bold text-green-600 dark:text-green-400 mb-1 uppercase tracking-wide">Result</p><p className="text-sm font-semibold text-black dark:text-white">{uc.outcome}</p></div><div className="flex gap-4 mt-4">{uc.metrics.map((m) => (<div key={m.label} className="text-center"><div className="text-xl font-black text-primary">{m.value}</div><div className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">{m.label}</div></div>))}</div></div>))}</div></Container></Section>
